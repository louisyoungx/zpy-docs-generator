...
import json
import re
from config import DICS_PATH, DICS_NAME
...
# DictCsv
#----------------------------------------------------------------------
class LocalDictionary(object):

    def __init__ (
            self,
            filename = DICS_PATH + '/' + DICS_NAME,
            codec = 'utf-8'
        ):
        self.__csvname = None
    ...
    # 获取词义列表
    def get(self, word):
        info = self.query(word)
        words = []
        try:
            wordLines = info['translation'].split('\n')
            for wordLine in wordLines:
                wordLine = re.sub('^.+\.\s', '', wordLine)
                wordLine = re.sub('\[.+\]', '', wordLine)
